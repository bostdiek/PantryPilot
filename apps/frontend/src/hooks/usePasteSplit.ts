...
interface UsePasteSplitOptions {
  onInsertSteps: (targetIndex: number, steps: string[], replaceEmpty?: boolean) => void;
...
export function usePasteSplit({ onInsertSteps, onReplaceStep, getCurrentStepValue }: UsePasteSplitOptions) {
  ...
  const handleInstructionPaste = useCallback((e: ClipboardEvent<HTMLTextAreaElement>, idx: number) => {
  ...
  const handlePasteSplitConfirm = useCallback((steps: string[]) => {
    const targetIdx = pasteSplitModal.targetIndex;
    
    // Guard against invalid targetIndex - delegate to parent for validation
    if (targetIdx < 0) {
      console.warn(`Invalid target index ${targetIdx}, cannot confirm paste split`);
      return;
    }
  ...
  const handlePasteAsSingle = useCallback((content: string) => {
    const targetIdx = pasteSplitModal.targetIndex;
    
    // Guard against invalid targetIndex
    if (targetIdx < 0) {
      console.warn(`Invalid target index ${targetIdx}, cannot paste as single step`);
      return;
    }
