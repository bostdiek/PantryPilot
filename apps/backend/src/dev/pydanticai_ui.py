...
# Global session factory - lazily initialized per event loop
_engine = None
...
async def ensure_dev_user() -> tuple[
    User, AsyncSession, async_sessionmaker[AsyncSession]
]:
    """Ensure a 'dev' user exists with location for testing weather tool.

    Returns:
        Tuple of (User, AsyncSession, session_factory) for dev tooling.
    """
    ...
    async with AsyncSessionLocal() as db:
        ...
        # Return the user and a fresh session for the agent to use
        session = AsyncSessionLocal()
        return dev_user, session, AsyncSessionLocal
...
def main() -> None:
    ...
    dev_user, _, _ = asyncio.run(ensure_dev_user())
